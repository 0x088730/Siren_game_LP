  ...
  return (
    <div className="fixed w-full h-20 bg-[#8d70ff] z-10 flex items-center shadow-2xl justify-between">
      ...
      <div className="w-1/2 h-full text-right xl:hidden">
        ...
        <div
          className={`absolute right-0 w-64 text-center xl:hidden bg-[#8d70ff] text-3xl transition-transform duration-500 transform ${
            showFlag.menu ? "translate-x-0" : "translate-x-full"
          }`}
        >
          {showFlag.menu === true &&
            menuList.map((menu) => {
              return (
                <div
                  key={menu}
                  className={`px-4 py-2 cursor-pointer hover:bg-[#3d208e] hover:text-white duration-700 ${
                    currentMenu === menu
