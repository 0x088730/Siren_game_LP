      ...
      <div className="absolute flex items-center justify-start w-5/6 h-full space-x-20">
        ...
        <div className="flex w-5/6 duration-1000 h-5/7"  ref={characterViewRef}>
          ...
          <div className="justify-between flex-1 w-1/2 text-white h-4/4" >
            ...
            <div className="relative flex w-full space-x-10">
            {characterList.map((no)=><img style= {{width:'30%', height: 'auto'}} key={no} src={"assets/images/characters/action/" + no + "-1.jpg"}
            className={currentCharacter!==no?"hidden":"content"}
            alt=""/>)}
              {characterList.map((no)=><img style= {{width:'30%', height: 'auto'}} key={no} src={"assets/images/characters/action/" + no + "-2.jpg"}
            className={currentCharacter!==no?"hidden":"content"}
            alt="" />)}
            {characterList.map((no)=><img style= {{width:'30%', height: 'auto'}} key={no} src={"assets/images/characters/action/" + no + "-3.jpg"}
            className={currentCharacter!==no?"hidden":"content"}
            alt="" />)}
            </div>
          </div>
